...
void MGEDemo::_initializeScene() {
	...
	//will store the level width and length, should be moved into a scene class lateron
	int levelWidth;
	int levelHeight;
	...
	for(unsigned i = 0; i < levelMap.size(); i++) {
		...
		if(tileProperty == tileProp::Uncolored || tileProperty == tileProp::PlayerSpawn) {
			GameObject* tile = new GameObject("tile" + std::to_string(i), glm::vec3(-(col - (float)levelWidth / 2.0f) * tileSpace, 0, (row - (float)levelHeight / 2.0f) * tileSpace));
